...
def save_plot_figure_2disk(dsg1=False,plotname="outfig"):
  """
  TODO: plotname TBD
  """
  #  plt.tight_layout()
  F = plt.gcf()
  DPI = F.get_dpi() #plt.axis('off')
  if dsg1:
...
def add_to_prod_rules(lhs, rhs, s):
    letter='a'
    d = {}
...
def build_graph_of_interest(grph_name=""):
	debug = False
	print
	print "--------------------"
	print "------ Graphs ------"
	...
	if grph_name == "Board Example":
		...
		G.add_edge(5,7)
		G.add_edge(6,7)
	elif grph_name == "Karate Club":
		G = nx.karate_club_graph()
	elif grph_name == "Random: ErdosRenyi":
		n=34
		m=68 # 20 edges
		G=nx.gnm_random_graph(n,m) #binomial_graph which is also somtims called the Erdos-Renyi graph.
	elif grph_name == "Random: NwmnWattsStrogatz":
		n=34	
		k=2
		p=0.5
		G = nx.newman_watts_strogatz_graph(n, k, p)
	"""The watts_strogatz is a ring, we want something without loops"""
	#elif grph_name == "Small World":
	#	G = nx.watts_strogatz_graph(34, 2, 0)
...
#           new_idx = {}
#           n_rhs =str(control_rod(prod_rules[lhs_str].items())).lstrip("(").rstrip(")")
...
# 
# #      if run >2: break
# #          graphlet_node_cnt.append(newG.number_of_nodes())
# 
# #          if(xxx<ttt):
# #            print "Shrink"
# #          ttt = xxx
# 
# #      print chartvis.bfs_eff_diam(newG, 50, .90)
# #print run, 'graph size for run:', len(eff_dia_gph)
...
def main ():
	...
	global options, args
	# TODO: Let's something more interesting here...
	print 'Hello world!'
	
# 	Graph = build_graph_of_interest("Board Example")
# 	Graph = build_graph_of_interest("Karate Club")
# 	Graph = build_graph_of_interest("Random: ErdosRenyi")
	Graph = build_graph_of_interest("Random: NwmnWattsStrogatz")
