...
def network_properties(orig, net_mets, synth_graphs_lst, name='', out_tsv=False, plotSave=False):
		...
		if 'degree' in net_mets:
				...
				if len(orig__Deg)>100: orig__Deg= orig__Deg.sample(70).sort_index()
				print '... Orig G'
				...
				if len(synth_Deg)>100: synth_Deg = synth_Deg.sample(70).sort_index()
				print '\n... HRG G'
		...
		if 'hops' in net_mets:
			...
			if len(orig__Hop_Plot)>100: orig__Hop_Plot = orig__Hop_Plot.sample(70).sort_index()
			synth_Hop_Plot = hop_plot_multiples(synth_graphs_lst)
			if len(synth_Hop_Plot)>100: synth_Hop_Plot= synth_Hop_Plot.sample(70).sort_index()
		...
		if 'clust' in net_mets:
			...
			if len(orig__clust_coef)>=100: orig__clust_coef = orig__clust_coef.sample(70).sort_index()
			synth_clust_coef = clustering_coefficients_multiples(synth_graphs_lst)
			if len(synth_clust_coef)>=100: synth_clust_coef = synth_clust_coef.sample(70).sort_index()
			...
			try:
				...
				if out_tsv: gb['cc'].mean().to_csv('Results/clust_hrg_{}.tsv'.format(name),sep='\t')
			except Exception, e:
				print str(e)
		...
		if 'assort' in net_mets:
			...
			if len(orig__assort) >= 100: orig__assort = orig__assort.sample(70).sort_index()
			synth_assort = assortativity_coefficients_multiples(synth_graphs_lst)
			if len(synth_assort) >= 100: synth_assort = synth_assort.sample(70).sort_index()

			gb = orig__assort.groupby(['k'])
			gb[1].mean().plot(ax=ax3, marker='o', ls="None", markeredgecolor="w", color='b',	alpha=0.8 )
			if out_tsv: gb[1].mean().to_csv('Results/assort_orig_{}.tsv'.format(name),sep='\t')
			print '... Orig G'
			...
			if out_tsv:		gb[1].mean().to_csv('Results/assort_hrg_{}.tsv'.format(name),sep='\t')
			print '\n... Synth G'
			print gb[1].mean().to_string(header=False)
		...
		if 'kcore' in net_mets:
			...
			if len(orig__kcore) >= 100: orig__kcore= orig__kcore.sample(70).sort_index()
			synth_kcore = kcore_decomposition_multiples(synth_graphs_lst)
			if len(synth_kcore) >= 100: synth_kcore= synth_kcore.sample(70).sort_index()
		...
		if 'eigen' in net_mets:
			...
			if len(orig__eigenvec) >= 100: orig__eigenvec= orig__eigenvec.sample(70).sort_index()
			...
			if len(synth_eigenvec) >= 100: synth_eigenvec= synth_eigenvec.sample(70).sort_index()
			...
			print '... Orig G', orig__eigenvec.shape
			print orig__eigenvec.mean(axis=1).to_string(header=False)
			print '\n... Synth G', synth_eigenvec.shape
...
def degree_ecdf(origG, glist, lable, dbg=False):
	print 
	if dbg: print "Degree ECDF"
	dist = nx.degree_histogram(origG)[1:]
	ecdf_composite_df= pd.DataFrame(*ecdf(dist), columns=['orig'])


	for j,c in enumerate(glist):
		dist = nx.degree_histogram(c)[1:]
		synth_ecdf = pd.DataFrame(*ecdf(dist), columns=['s'+str(j)])
		ecdf_composite_df=pd.merge(ecdf_composite_df, synth_ecdf, how='outer', left_index=True, right_index=True)
		
	return ecdf_composite_df
	

def degree_CDF_test(origG, glist, label):
    print
...
#		output = popen.stdout.read()
		...
		#timer = Timer(6000, kill_proc, [proc])
		#try:
			#timer.start()
