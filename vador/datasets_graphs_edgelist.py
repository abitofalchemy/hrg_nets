...
def graph_name(fname):
	return [x for x in os.path.basename(fname).split('.') if len(x) > 3][0]

def pickled_graph_exists(fname):
	g_name = graph_name(fname)
	f = "datasets/{}.pickle".format(g_name)
...
def load_edgelist(fname):
	...
	g_name = graph_name(fname)
	if pickled_graph_exists(fname):
		f = "datasets/{}.pickle".format(g_name)
		G = pickle.load(open(f, "rb"))	#load_gpickle_file
	else: 
		df = tdf.Pandas_DataFrame_From_Edgelist([fname])[0]
		if len(df.columns) ==3: 
